...
class TestGame(unittest.TestCase):
    ...
    def test_play_round_perfect(self):
        settings = base_settings()
        with mock.patch.object(game.core, "build_sequence", return_value=["red"]), mock.patch.object(game.core, "add_step", side_effect=lambda seq: seq.append("blue") or seq), mock.patch("builtins.input", StackedInput(["r", "r b", "r b b", "r b b b", "r b b b b", "r b b b b b"])):
            result, text = self.capture(game.play_round, settings)
        self.assertEqual(result, 435)
        self.assertIn("Rating: perfect", text)

    def test_play_round_hint_then_correct(self):
        settings = base_settings()
        with mock.patch.object(game.core, "build_sequence", return_value=["red"]), mock.patch("builtins.input", StackedInput(["hint", "r", "wrong"])):
            result, text = self.capture(game.play_round, settings)
        self.assertEqual(result, 17)
        self.assertIn("Hint", text)
        self.assertIn("Wrong", text)

    def test_play_round_duplicate_hint(self):
        settings = base_settings()
        with mock.patch.object(game.core, "build_sequence", return_value=["red"]), mock.patch("builtins.input", StackedInput(["hint", "hint", "wrong"])):
            result, text = self.capture(game.play_round, settings)
        self.assertEqual(result, 0)
        self.assertIn("Hint already used", text)

    def test_play_round_quits(self):
        settings = base_settings()
        with mock.patch.object(game.core, "build_sequence", return_value=["red"]), mock.patch("builtins.input", StackedInput(["q"])):
