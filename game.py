...
def play_round(settings):
    ...
    sequence = core.build_sequence(cfg["start"])
    completed = 0
    total_score = 0
    streak = 0
    ...
    for round_index in range(1, cfg["rounds"] + 1):
        if round_index > 1:
            core.add_step(sequence)
        used_hint = False
        _print(t(lang, "round", round=round_index, total=cfg["rounds"]))
        _print(t(lang, "sequence", sequence=core.sequence_text(sequence)))
        while True:
            answer = input(t(lang, "answer_prompt")).strip().lower()
            if answer == "q":
                raise QuitGame()
            if answer == "hint":
                if used_hint:
